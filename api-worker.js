...
export default {
  async fetch(request) {
    ...
    if (url.pathname === '/health') {
      return Response.json({ ok: true, runtime: 'cloudflare-api-worker' }, {
    ...
    if (url.pathname.startsWith('/api/')) {
      ...
      if (!endpoint || endpoint.includes('/')) {
        return Response.json({ error: '接口不存在', code: 'NOT_FOUND' }, { status: 404, headers: CORS });
      }
      const response = await handleApiRequest(request, endpoint);
      const headers = new Headers(response.headers);
      for (const [key, value] of Object.entries(CORS)) headers.set(key, value);
      return new Response(response.body, { status: response.status, headers });
    }

    return Response.json({ ok: true, service: 'zaotu-ai-api' }, { headers: CORS });
  },
};
