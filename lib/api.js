...
const TEXT_FREE_IMAGE_RULE = '底图绝对禁止出现任何文字、汉字、字母、数字、序号、页码、01/06、标签、标题、制作指令、Logo、水印或二维码；只生成干净摄影画面，买家文案由网页后期排版。';
function userHardRequirement(note) { const value = String(note || '').trim().slice(0, 2000); return value ? `用户最高优先级硬性要求：${value}。这些内容是制作指令，严禁作为画面文字；必须逐条执行背景、颜色、尺寸、数量和禁用项。` : ''; }
function seriesVisualContract(note) { return `整套统一视觉合同：全套使用同一主辅色体系、同一背景语言、同一光线方向和色温、同一商品材质质感、同一镜头语言与留白规则。只允许画面任务、商品摆位和机位按分镜变化，禁止每张另起一种摄影风格。${/黑色背景|纯黑背景|深黑背景/.test(String(note || '')) ? '全套采用黑色或深炭黑背景体系。' : ''}`; }
...
function projectSeriesRoles(suiteType, includeWhite) {
  return !includeWhite && SCENE_ONLY_SERIES_ROLES[suiteType] ? SCENE_ONLY_SERIES_ROLES[suiteType] : SERIES_ROLES[suiteType];
}
...
async function analyze(request, input, id) {
  ...
  const explicitProjectMode = ['main', 'detail', 'sku'].includes(input.projectMode);
  const projectMode = explicitProjectMode ? input.projectMode : 'legacy';
  const includeWhite = projectMode === 'sku' || (projectMode === 'main' ? input.includeWhite === true : suiteType !== 'vertical' && input.includeWhite !== false);
  ...
  const skuSpecs = (Array.isArray(input.skuSpecs) ? input.skuSpecs : []).slice(0, 10);
  const mainRoles = ['爆款首图核心卖点', '同背景痛点场景', '同背景真实使用', '同背景前后对比', '核心功能与材质证据', '转化收尾主视觉'];
  const roles = projectMode === 'legacy' ? projectSeriesRoles(suiteType, includeWhite) : projectMode === 'detail' ? ['详情首屏核心价值', '核心功能与结构说明', '材质尺寸与细节证据', '真实场景与转化收尾'] : projectMode === 'sku' ? skuSpecs.map(spec => `${Math.max(1, Number(spec?.quantity) || 1)}个 · ${String(spec?.name || '规格装').slice(0, 24)}`) : mainRoles.slice(includeWhite ? 1 : 0);
  ...
  const styleContract = seriesVisualContract(input.userNote);
  const outputMode = projectMode === 'legacy' ? (includeWhite ? '白底首图由 white_prompt 单独生成，不要把白底图重复放进 series。' : '本套不生成白底图，全部输出名额都必须用于彼此不同的真实场景图；white_prompt 必须返回空字符串。') : projectMode === 'sku'
    ? `生成单个商品母版并由网页排版真实数量。背景选择：${String(input.skuBackground || 'auto')}；商品尺寸：${String(input.skuDimensions || '未提供')}；买家主卖点：${String(input.skuSellingPoint || '需从可见事实提炼')}。`
    : projectMode === 'detail' ? '固定4张连续详情分镜；每屏只讲一个可验证购买重点。'
    : includeWhite ? '白底首图由 white_prompt 单独生成，series规划其余5张。' : '固定6张场景主图，不生成白底图。';
  const prompt = `你是中国大陆电商百货商品视觉策划师、买家文案策划师和图像模型提示词专家。先从商品证据图分析商品身份、真实功能、结构材质、目标买家、使用痛点和购买理由，再为${targetModel}制定逐张方案。\n前 ${productImages.length} 张是同一商品的商品证据图；后 ${styleImages.length} 张是场景/风格参考图，只作构图和环境参考。商品证据优先，场景参考不得覆盖商品事实。\n平台：${PLATFORM_RULES[platform]}\n方向：${DIRECTION_RULES[direction]}\n输出：${outputMode}\n${userHardRequirement(input.userNote)}\n${styleContract}\n${TEXT_FREE_IMAGE_RULE}\nseries 必须正好${roles.length}项：\n${roleLines}\n买家文案规则：主图模式下，只有“同背景真实使用”这一张纯场景图允许 headline 和 subheadline 为空；其余主图必须先根据商品事实写出非空的 headline 与 subheadline。headline 用4—14字直接表达一个顾客购买理由，subheadline 用8—24字解释利益、痛点或可见证据。不得使用“核心卖点、使用痛点、购买理由、第几张、商品证据图、请核对、完成转化”等栏目名或商家内部话术。详情页各屏同样必须有买家可读文案。\n只输出合法JSON：{"product_name":"准确商品通用名称","product_summary":"商品身份摘要","reference_strategy":"参考分配策略","series_style":"统一配色、光线、背景、镜头与版式合同","white_prompt":"商品母版无字提示词","selling_points":["经商品事实支持的买家购买理由"],"series":[{"label":"固定任务名称","product_refs":[1],"style_ref":1,"headline":"非纯场景必填的买家短卖点","subheadline":"非纯场景必填的买家利益、痛点或证据","prompt":"完整中文无字底图提示词"}]}。只允许画面任务、商品摆位和机位按分镜变化；全套色温、配色、质感和摄影语言必须一致。禁止拼图、分屏、多宫格、内部流程词、序号、页码、制作指令和虚构卖点。`;
  ...
  const series = Array.isArray(raw.series) ? raw.series.slice(0, roles.length).map((item, index) => {
    ...
    return { label: roles[index], product_refs: productRefs.length ? productRefs : [index % productImages.length + 1], style_ref: styleRef, headline: String(item?.headline || '').trim().slice(0, 40), subheadline: String(item?.subheadline || '').trim().slice(0, 80), prompt: String(item?.prompt || '').trim().slice(0, 7000) };
  }) : [];
  if (series.length !== roles.length || series.some(item => !item.prompt)) return failure(502, '提示词方案数量不完整，请重新生成', 'PROMPT_PLAN_INCOMPLETE', id);
  return json(200, { product_name: String(raw.product_name || '商品').trim().slice(0, 50), product_summary: String(raw.product_summary || `已综合识别 ${productImages.length} 张商品证据图中的商品主体与不同角度`).trim().slice(0, 500), reference_strategy: String(raw.reference_strategy || '商品证据图锁定身份，场景参考只提供构图与环境。').trim().slice(0, 500), series_style: String(raw.series_style || styleContract).trim().slice(0, 1000), selling_points: Array.isArray(raw.selling_points) ? raw.selling_points.map(item => String(item).slice(0, 60)).slice(0, 8) : [], white_prompt: includeWhite ? String(raw.white_prompt || `${styleContract}${userHardRequirement(input.userNote)}${TEXT_FREE_IMAGE_RULE}生成单个商品保真母版。`).trim().slice(0, 5000) : '', series, roles, provider: usedProvider, model: usedModel, fallback: usedProvider !== textProvider });
}
...
async function seedream(arkKey, image, prompt, imageModel, aspect) {
  ...
  const response = await timedFetch(ARK_IMAGE_URL, {
    method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${arkKey}` },
    body: JSON.stringify({ model: imageModel || DEFAULT_IMAGE_MODEL, prompt: `${prompt}\n${TEXT_FREE_IMAGE_RULE}\n输出要求：${rule}；只生成一张完整图片；严格保持参考商品结构、颜色、数量、包装标识位置。`, image: normalizeImage(image), size: '2K', response_format: 'url', watermark: false }),
  }, 180000);
...
async function seedreamCreate(arkKey, images, prompt, imageModel, aspect) {
  ...
  const body = { model: imageModel || DEFAULT_IMAGE_MODEL, prompt: `${prompt}\n${TEXT_FREE_IMAGE_RULE}\n输出要求：${rule}；只生成一张完整图片；禁止无关品牌元素。`, size: '2K', response_format: 'url', watermark: false };
...
async function geminiImage(geminiKey, images, prompt, aspect, model) {
  ...
  const input = [{ type: 'text', text: `${prompt}\n${TEXT_FREE_IMAGE_RULE}\n只输出一张完整图片；严格遵循参考图身份与商品结构；禁止无关品牌。` }];
  for (const image of images.slice(0, 10)) input.push({ type: 'image', ...dataImage(image) });
...
async function openaiCreate(openaiKey, images, prompt, aspect, model) {
  ...
  if (!images.length) {
    const response = await timedFetch(OPENAI_IMAGE_GENERATE_URL, {
      ...
      body: JSON.stringify({ model: model || DEFAULT_OPENAI_IMAGE_MODEL, prompt: `${prompt}\n${TEXT_FREE_IMAGE_RULE}`, size, quality: 'medium', output_format: 'jpeg' }),
    }, 210000);
  ...
  form.append('prompt', `${prompt}\n${TEXT_FREE_IMAGE_RULE}`); form.append('size', size); form.append('quality', 'medium'); form.append('output_format', 'jpeg'); form.append('output_compression', '88'); form.append('background', 'opaque');
...
async function qwenImage(qwenKey, images, prompt, aspect, model, input = {}) {
  ...
  content.push({ text: `${prompt}\n${TEXT_FREE_IMAGE_RULE}\n只输出一张完整图片；保持参考商品结构、颜色和包装标识位置真实；禁止拼图、分屏和无关品牌。` });
  const response = await timedFetch(qwenImageUrl(input), {
    ...
    body: JSON.stringify({
      ...
      parameters: { n: 1, negative_prompt: '拼图，分屏，多宫格，低清晰度，汉字，英文，字母，数字，序号，页码，01/06，标题，标签，制作指令，二维码，水印，无关品牌', prompt_extend: true, watermark: false, size: qwenImageSize(aspect) },
    }),
  }, 210000);
...
async function toolImage(request, input, id) {
  ...
  const prompt = `${seriesVisualContract(input.userNote)}\n${userHardRequirement(input.userNote)}\n${TEXT_FREE_IMAGE_RULE}\n${base}\n已确认提示词：${editedPrompt}`;
  try {
...
async function toolPrompt(request, input, id) {
  ...
  const styleContract = seriesVisualContract(note);
  const scenePlan = sceneRoles.length ? `\n本组固定画面任务（必须逐项对应，不得调换或合并）：\n${sceneRoles.map((role, index) => `${index + 1}. ${role}`).join('\n')}\n每项必须是一张独立单场景图片，禁止拼图、分屏、多宫格。${styleContract}只允许画面任务、商品摆位和机位按分镜变化，禁止每张改变配色、光线色温和摄影质感。` : '';
  const prompt = `你是商业图片工作流的提示词总监。先理解用户素材与任务，再为${targetModel}编写真正可执行的图片提示词。\n\n工具目标：${base}\n素材说明：${String(input.inputHint || `共${images.length}张参考图`).slice(0, 500)}\n${userHardRequirement(note)}\n输出数量：${outputCount}\n输出比例：${aspect}${scenePlan}\n${TEXT_FREE_IMAGE_RULE}\n\n只输出合法JSON，不要Markdown：{"summary":"对素材、目标和风险的简短理解","series_style":"整套统一视觉合同","prompts":[{"label":"结果名称","headline":"经商品事实支持的买家卖点，可为空","subheadline":"买家利益点或痛点，可为空","prompt":"完整无字底图提示词"}]}。prompts必须正好${outputCount}项。每条提示词要明确主体身份与保真项、构图、镜头、光线、背景、材质、颜色、输出比例和禁止项；不得把制作指令写进画面。不得臆造商品功能、认证、价格、销量或第三方品牌。`;
  ...
  for (const provider of attempts) {
    try {
      ...
      return json(200, {
        ...
        series_style: String(raw.series_style || styleContract).slice(0, 1000),
        prompts,
        model,
        provider,
        fallback: provider !== requestedProvider,
...
async function whiteImage(request, input, id) {
  ...
  try {
    const finalPrompt = `${seriesVisualContract(input.userNote)}\n${userHardRequirement(input.userNote)}\n${TEXT_FREE_IMAGE_RULE}\n${input.prompt}`;
    const result = await imageWithFallback(provider, { ark: arkKey, openai: openaiKey, gemini: geminiKey, qwen: qwenKey }, images, finalPrompt, 'square', input, true);
...
async function seriesImage(request, input, id) {
  ...
  try {
    const finalPrompt = `${seriesVisualContract(input.userNote)}\n${userHardRequirement(input.userNote)}\n${TEXT_FREE_IMAGE_RULE}\n${input.prompt}`;
    return json(200, await imageWithFallback(provider, { ark: arkKey, openai: openaiKey, gemini: geminiKey, qwen: qwenKey }, images, finalPrompt, aspect, input, true));
...
async function qc(request, input, id) {
  ...
  const platform = ['taobao', 'pdd', '1688'].includes(input.platform) ? input.platform : 'taobao';
  const prompt = `你是中国大陆电商商品图严格质检员。前 ${productImages.length} 张是用户上传的原始商品证据图${input.whiteImageBase64 ? '，倒数第2张是系统生成的商品母版' : ''}，最后1张是尚未经过网页排字的待检底图。任务：${String(input.taskLabel || '商品图').slice(0, 40)}。平台要求：${PLATFORM_RULES[platform]}\n必须以原始商品证据图为最高事实依据。严格检查商品形状、比例、颜色、材质、包装结构、标签位置、配件数量、穿模和虚构卖点。文字是硬失败项：待检底图中出现任何可辨识汉字、字母、数字、序号、01/06、页码、标题、标签、制作指令、二维码、水印或新Logo，pass必须为false，不得因画面美观放行。只输出合法JSON：{"pass":true,"score":0,"summary":"不超过40字中文结论","correction":"不通过时给图片模型的不超过180字精准修改要求，通过则为空"}。只有评分不低于90、商品完全保真且底图完全无字时才能通过。`;
  ...
  const pass = raw.pass === true && score >= 90;
