...
async function analyze(request, input, id) {
  ...
  for (const provider of attempts) {
    try {
      if (provider === 'gemini') {
        ...
        raw = extractObject(await geminiText(geminiKey, prompt, [input.imageBase64], usedModel));
      } else {
        usedModel = input.visionModel || DEFAULT_VISION_MODEL;
        raw = extractObject(await arkText(arkKey, prompt, [input.imageBase64], usedModel, { json: true, temperature: 0.25, maxTokens: suiteType === 'full' ? 4600 : 3000 }));
      }
  ...
  if (!raw) return failure(lastError?.status || 502, lastError?.userMessage || '商品理解与提示词规划暂时未完成', lastError?.message || 'ANALYZE_FAILED', id);
...
async function toolPrompt(request, input, id) {
  ...
  for (const provider of attempts) {
    try {
      ...
      return json(200, {
        summary: String(raw.summary || '已完成素材理解与提示词规划').slice(0, 500),
        prompts,
        model,
        provider,
  ...
  return failure(lastError?.status || 502, lastError?.userMessage || '提示词生成暂时未完成，请重试', lastError?.message || 'TOOL_PROMPT_FAILED', id);
}
...
async function connectionTest(request, input, id) {
  ...
  try {
    ...
    if (provider === 'doubao') {
      ...
      return json(200, { ok: Boolean(result), provider, model, message: '火山方舟文字与提示词连接正常' });
    }
    if (provider === 'openai') {
