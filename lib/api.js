...
const MAX_DOWNLOAD_IMAGE_BYTES = 25 * 1024 * 1024;
const DOWNLOAD_IMAGE_HOST_SUFFIXES = ['volces.com', 'volcengine.com', 'volccdn.com', 'aliyuncs.com'];
...
const PRODUCT_REALISM_LOCK = '真实感与零错位是硬约束：主体轮廓必须连续清晰，禁止重影、双轮廓、复制边缘、抠图接缝、融化、扭曲、部件缺失或重复；商品、内装物、台面及人物手部必须符合真实遮挡和支撑关系，禁止悬浮、下陷、穿插、裁切错位或手指穿模；透视、消失线、尺度、接触阴影、反射、折射、透明度和光源方向必须物理一致；标签、Logo和部件位置只能沿原结构自然透视，不得漂移或凭空新增。';
...
const DETAIL_PAGE_ROLES = [
  '详情首屏核心价值',
  '核心功能与结构说明',
  '材质尺寸与细节证据',
  '真实场景与转化收尾',
];
const DEFAULT_SKU_SPECS = [
  { quantity: 1, name: '试用装' },
  { quantity: 4, name: '实惠小套装' },
  { quantity: 6, name: '宿舍够用装' },
  { quantity: 8, name: '家庭实惠装', recommended: true },
  { quantity: 10, name: '大容量囤货装' },
];
const SCENE_ONLY_SERIES_ROLES = {
  square: ['核心卖点场景', '日常使用场景', '材质做工细节', '尺寸使用感知', '转化留白构图'],
  full: ['核心卖点场景', '日常使用场景', '第二使用环境', '材质做工细节', '尺寸使用感知', '首屏卖点概览', '核心结构功能', '材质工艺证明', '尺寸适配说明', '使用场景收尾'],
};
function normalizeSkuSpecs(value) {
  const source = Array.isArray(value) && value.length ? value : DEFAULT_SKU_SPECS;
  return source.slice(0, 10).map((item, index) => ({
    quantity: Math.max(1, Math.min(20, Number(item?.quantity) || index + 1)),
    name: String(item?.name || `${index + 1}个装`).trim().slice(0, 24),
    recommended: Boolean(item?.recommended),
  }));
}
function projectSeriesRoles(suiteType, includeWhite, projectMode = 'main', detailCount = 4, skuSpecs = []) {
  if (projectMode === 'detail') return DETAIL_PAGE_ROLES;
  if (projectMode === 'sku') return normalizeSkuSpecs(skuSpecs).map(item => `SKU ${item.quantity}个·${item.name}`);
  return includeWhite ? MAIN_PRODUCT_ROLES.slice(1) : MAIN_PRODUCT_ROLES;
}
...
function downloadableImageUrl(value) {
  try {
    ...
    if (!DOWNLOAD_IMAGE_HOST_SUFFIXES.some(suffix => host === suffix || host.endsWith(`.${suffix}`))) return null;
    return url;
  } catch {
    return null;
  }
...
async function downloadImage(input, id) {
  const url = downloadableImageUrl(input.url);
  if (!url) return failure(400, '该图片地址不属于已连接的生图服务', 'IMAGE_DOWNLOAD_URL_REJECTED', id);
  try {
    const response = await timedFetch(url, { headers: { Accept: 'image/*' } }, 60000);
    if (!response.ok) return failure(502, `图片源站暂时无法下载（HTTP ${response.status}）`, 'IMAGE_DOWNLOAD_UPSTREAM_FAILED', id);
    const mime = String(response.headers.get('content-type') || '').split(';')[0].trim().toLowerCase();
    if (!mime.startsWith('image/')) return failure(502, '图片源站返回的不是图片文件', 'IMAGE_DOWNLOAD_TYPE_INVALID', id);
    const declared = Number(response.headers.get('content-length') || 0);
    if (declared > MAX_DOWNLOAD_IMAGE_BYTES) return failure(413, '图片文件超过 25MB，无法打包下载', 'IMAGE_DOWNLOAD_TOO_LARGE', id);
    const bytes = await response.arrayBuffer();
    if (bytes.byteLength > MAX_DOWNLOAD_IMAGE_BYTES) return failure(413, '图片文件超过 25MB，无法打包下载', 'IMAGE_DOWNLOAD_TOO_LARGE', id);
    return new Response(bytes, { status: 200, headers: { ...CORS, 'Content-Type': mime, 'Content-Length': String(bytes.byteLength), 'Cache-Control': 'private, max-age=300' } });
  } catch (error) {
    return failure(502, '图片源站连接失败，请立即重试下载', error.message || 'IMAGE_DOWNLOAD_FETCH_FAILED', id);
  }
}
...
async function analyze(request, input, id) {
  ...
  const projectMode = ['main', 'detail', 'sku'].includes(input.projectMode) ? input.projectMode : 'main';
  const suiteType = ['square', 'vertical', 'full'].includes(input.suiteType) ? input.suiteType : 'square';
  const detailCount = 4;
  const mainAspect = input.mainAspect === 'portrait' ? 'portrait' : 'square';
  const skuSpecs = normalizeSkuSpecs(input.skuSpecs);
  const includeWhite = projectMode === 'sku' ? true : projectMode === 'detail' ? false : suiteType !== 'vertical' && input.includeWhite !== false;
  ...
  const roles = projectSeriesRoles(suiteType, includeWhite, projectMode, detailCount, skuSpecs);
  ...
  const outputMode = projectMode === 'sku'
    ? `SKU规格模式：先生成一张单件白底商品基准，随后由网页确定性复制排版。套餐依次为：${skuSpecs.map(item => `${item.quantity}个${item.name}${item.recommended ? '（推荐）' : ''}`).join('、')}。series 只规划每个SKU的短文案与排版重点，不要求图像模型直接数出多个商品。`
    : projectMode === 'detail'
      ? `详情页模式：固定生成4张2:3竖版精简详情图，依次解决核心价值、功能结构、材质尺寸证据和真实场景收尾；画面先生成无字底图，标题和副标题由网页设计层叠加；white_prompt 返回空字符串。`
      : includeWhite
        ? `商品主图模式：固定输出6张${mainAspect === 'portrait' ? '3:4竖版' : '1:1正方形'}主图。第1张白底首图由 white_prompt 单独生成，series规划其余5张；第2—4张必须共用同一套简洁背景、光向、色温和台面材质。`
        : `商品主图模式：固定输出6张${mainAspect === 'portrait' ? '3:4竖版' : '1:1正方形'}主图；第2—4张必须共用同一套简洁背景、光向、色温和台面材质，只改变商品摆位、收纳物、少量道具与机位；white_prompt 必须返回空字符串。`;
  const modeRules = projectMode === 'sku'
    ? 'SKU图不是场景图。所有规格共用同一单品角度、比例、材质和版式；数量必须与套餐一致，数量由程序复制商品基准保证，不让生图模型猜数。headline突出“数量+套餐名”，subheadline只放必要规格。'
    : projectMode === 'detail'
      ? '详情页固定4张，按首屏价值→功能结构→材质尺寸证据→真实场景收尾形成完整精简叙事。每张headline只讲一个利益点，subheadline补充一个事实；layout_notes说明文字安全区。'
      : '商品主图固定6张。第2—4张属于统一背景组：统一米白或浅灰背景、统一台面材质、统一主光方向与色温、统一低密度道具语言；每张仅改变商品摆位、收纳内容、1—3件相关道具和机位，绝不能换房间或堆叠杂物。第1张突出核心卖点，第5张提供功能材质证据，第6张转化收尾。所有文案必须来自可见事实，不得虚构价格、折扣、销量或功效。';
  const prompt = `你是中国大陆电商商品分析师、主图与详情页策划师和图像模型提示词专家。必须先完成“产品事实分析”，再按指定模式制定逐张可执行方案。\n\n图片分组与顺序：前 ${productImages.length} 张是同一商品的商品证据图，分别可能是正面、侧面、背面、细节、配件或包装，用于综合锁定同一个商品的真实形状、比例、颜色、材质、尺寸、标签、标识、配件和角度；后 ${styleImages.length} 张是场景/风格参考图，只允许借鉴环境、构图、机位、光线和色调，不能把其中的商品、品牌、文字或人物身份带入结果。\n\n项目模式：${projectMode}\n目标平台策略：${PLATFORM_RULES[platform]}\n套图方向：${DIRECTION_RULES[direction]}\n输出策略：${outputMode}\n模式规则：${modeRules}\n用户补充：${String(input.userNote || '无').slice(0, 2000)}\n\n先区分三类信息：1）图片中能直接确认的事实；2）合理但需用户确认的推断；3）图片无法确认、禁止编造的信息。不得根据文件名认产品，不得把示例文案当成商品事实。\n\n主体几何是不可变事实：${PRODUCT_GEOMETRY_LOCK}\n${PRODUCT_REALISM_LOCK}\n只要证据图之间存在角度差异，也必须按同一实体的固定三维结构理解，不能把透视差异误判为尺寸变化。\n\nseries 必须正好${roles.length}项，顺序和 label 固定为：\n${roleLines}\n\n只输出合法JSON，不要Markdown：\n{"product_name":"不超过14字的准确商品通用名称，不使用文件名和编号","product_summary":"综合全部商品证据图得到的商品身份摘要","product_identity":{"category":"品类","shape":"外形结构","materials":["仅写可确认材质"],"colors":["颜色"],"dimensions":["可见尺寸"],"components":["组成部件"],"identity_markers":["跨图必须保持的识别特征"]},"verified_facts":["图片直接证实的事实"],"unknowns":["需要用户确认、不能编造的信息"],"selling_points":["由可见事实支持的卖点"],"use_cases":["合理使用场景"],"audience":["目标人群"],"reference_strategy":"商品证据与场景参考的分配方法","white_prompt":"需要单品白底基准时填写，否则为空字符串","series":[{"label":"固定任务名称","product_refs":[1],"style_ref":1,"headline":"不超过14字的中文主标题；少字画面可为空","subheadline":"不超过24字的事实型副标题；少字画面可为空","layout_notes":"说明标题强弱和文字安全区","prompt":"给${targetModel}的一张无字底图完整中文提示词"}]}\n\nproduct_refs 只能使用 1—${productImages.length}，每项最多2张；style_ref 在有场景图时使用 1—${Math.max(styleImages.length, 1)}，没有则为 null。主图第2—4项必须写入同一套背景母版描述，并在有风格图时统一 style_ref=1；背景固定为简洁墙面与单一台面、同一光向和色温，道具总数1—3件，仅允许商品摆位、收纳物和机位变化。主图其他项与详情页按各自信息任务变化构图。每次只生成一张独立画面，禁止拼图、分屏、多宫格或仅展示包装。必须保持商品真实，不重新设计、不增减功能、不遮挡主体。底图禁止直接生成文字、价格、促销角标、Logo、店铺名、二维码、联系方式、水印、边框、乱码、认证、比较元素和绝对化宣传；文案仅放入headline与subheadline，供网页后期排版。`;
  ...
  for (const provider of attempts) {
    try {
      const generated = await providerText(provider, textKeys, prompt, visionImages, input, { json: true, temperature: 0.2, maxTokens: projectMode === 'detail' ? 7600 : suiteType === 'full' ? 5200 : 4200 });
  ...
  const series = Array.isArray(raw.series) ? raw.series.slice(0, roles.length).map((item, index) => {
    ...
    const outputIndex = includeWhite ? index + 1 : index;
    const sharedBackground = projectMode === 'main' && [1, 2, 3].includes(outputIndex);
    const styleRef = sharedBackground && styleImages.length ? 1 : styleImages.length && Number(item?.style_ref) >= 1 && Number(item?.style_ref) <= styleImages.length ? Number(item.style_ref) : styleImages.length ? index % styleImages.length + 1 : null;
    const sharedRule = sharedBackground ? '本张属于主图第2—4张同背景组：统一柔和米白墙、浅原木或暖白单色台面、左上45度自然窗光、奶油暖白低饱和色温与柔和接触阴影；背景仅1—3件低存在感道具。背景墙、台面、光向、色温不得改变，仅改变商品摆位、收纳物与机位。' : '';
    return {
      label: roles[index],
      product_refs: productRefs.length ? productRefs : [index % productImages.length + 1],
      style_ref: styleRef,
      headline: String(item?.headline || roles[index]).trim().slice(0, 40),
      subheadline: String(item?.subheadline || '').trim().slice(0, 80),
      layout_notes: String(item?.layout_notes || '保留清晰文字安全区').trim().slice(0, 240),
      prompt: `${String(item?.prompt || '').trim()}\n${PRODUCT_GEOMETRY_LOCK}\n${PRODUCT_REALISM_LOCK}\n${sharedRule}`.trim().slice(0, 7000),
    };
  }) : [];
  if (series.length !== roles.length || series.some(item => !item.prompt)) return failure(502, '提示词方案数量不完整，请重新生成', 'PROMPT_PLAN_INCOMPLETE', id);
  const stringList = value => Array.isArray(value) ? value.map(item => String(item).trim().slice(0, 160)).filter(Boolean).slice(0, 16) : [];
  const identity = raw.product_identity && typeof raw.product_identity === 'object' ? raw.product_identity : {};
  return json(200, {
    product_name: String(raw.product_name || '商品').trim().slice(0, 50),
    product_summary: String(raw.product_summary || `已综合识别 ${productImages.length} 张商品证据图中的商品主体与不同角度`).trim().slice(0, 500),
    product_identity: {
      category: String(identity.category || '').trim().slice(0, 80),
      shape: String(identity.shape || '').trim().slice(0, 200),
      materials: stringList(identity.materials), colors: stringList(identity.colors), dimensions: stringList(identity.dimensions),
      components: stringList(identity.components), identity_markers: stringList(identity.identity_markers),
    },
    verified_facts: stringList(raw.verified_facts), unknowns: stringList(raw.unknowns), selling_points: stringList(raw.selling_points),
    use_cases: stringList(raw.use_cases), audience: stringList(raw.audience), project_mode: projectMode, sku_specs: projectMode === 'sku' ? skuSpecs : [],
    reference_strategy: String(raw.reference_strategy || `商品证据图用于锁定身份，场景参考图按张轮换构图、机位、光线与环境。`).trim().slice(0, 500),
    white_prompt: includeWhite ? `${String(raw.white_prompt || '综合全部商品证据图，只替换为纯白背景，严格保持商品外观、数量、标签和配件。').trim()}\n${PRODUCT_GEOMETRY_LOCK}\n${PRODUCT_REALISM_LOCK}`.slice(0, 5000) : '',
    series, roles, provider: usedProvider, model: usedModel, fallback: usedProvider !== textProvider,
  });
}

async function seedream(arkKey, image, prompt, imageModel, aspect) {
  const rule = aspect === 'vertical' ? '2:3竖版详情页构图' : aspect === 'portrait' ? '3:4竖版商品图构图' : '1:1正方形商品图构图';
...
async function seedreamCreate(arkKey, images, prompt, imageModel, aspect) {
  const rule = aspect === 'vertical' ? '2:3竖版构图' : aspect === 'portrait' ? '3:4竖版构图' : aspect === 'wide' ? '16:9横版构图' : '1:1正方形构图';
...
async function geminiImage(geminiKey, images, prompt, aspect, model) {
  const ratio = aspect === 'vertical' ? '2:3' : aspect === 'portrait' ? '3:4' : aspect === 'wide' ? '16:9' : '1:1';
...
async function openaiCreate(openaiKey, images, prompt, aspect, model) {
  const size = aspect === 'vertical' || aspect === 'portrait' ? '1024x1536' : aspect === 'wide' ? '1536x1024' : '1024x1024';
  if (!images.length) {
...
function qwenImageSize(aspect) {
  ...
  if (aspect === 'portrait') return '768*1024';
...
async function openai(openaiKey, image, prompt, aspect, model) {
  ...
  form.append('size', aspect === 'vertical' || aspect === 'portrait' ? '1024x1536' : '1024x1024');
...
async function whiteImage(request, input, id) {
  ...
  try {
    const aspect = input.aspect === 'portrait' ? 'portrait' : 'square';
    const result = await imageWithFallback(provider, { ark: arkKey, openai: openaiKey, gemini: geminiKey, qwen: qwenKey }, images, input.prompt, aspect, input, true);
...
async function seriesImage(request, input, id) {
  ...
  const aspect = input.aspect === 'vertical' ? 'vertical' : input.aspect === 'portrait' ? 'portrait' : 'square';
  try {
...
async function qc(request, input, id) {
  ...
  const platform = ['taobao', 'pdd', '1688'].includes(input.platform) ? input.platform : 'taobao';
  const prompt = `你是中国大陆电商商品图严格质检员。前 ${productImages.length} 张是用户上传的原始商品证据图${input.whiteImageBase64 ? '，倒数第2张是系统生成的白底基准图' : ''}，最后1张是待检生成图。任务：${String(input.taskLabel || '商品图').slice(0, 40)}。平台要求：${PLATFORM_RULES[platform]}\n\n必须以原始商品证据图为最高事实依据，白底基准只用于辅助核对。逐项检查：1）主体几何与身份：${PRODUCT_GEOMETRY_LOCK} 2）边缘与对位：轮廓连续、单一且清晰，无重影、双轮廓、复制边缘、抠图接缝、标签或部件漂移；3）物理接触与遮挡：商品、内装物、台面、道具和人物手部接触合理，无悬浮、下陷、穿插、裁切错位、手指穿模；4）材质与光影：纹理、透明度、反射、折射、接触阴影和光源方向一致；5）透视与尺度：消失线、相对大小、景深和空间位置一致；6）内容合规：无结构增减、复制主体、畸形手部、乱码、新价格、二维码、水印、第三方Logo或虚构宣传。任何一项失败都必须判定不通过。只输出合法JSON：{"pass":true,"alignment_pass":true,"realism_pass":true,"score":0,"summary":"不超过40字中文结论","issues":["具体问题"],"correction":"不通过时明确指出需恢复的结构、对位、接触或光影；不超过180字，通过则为空"}。只有评分不低于90、alignment_pass与realism_pass均为true且全部检查通过时pass才可为true。`;
  ...
  const alignmentPass = raw.alignment_pass === true;
  const realismPass = raw.realism_pass === true;
  const pass = raw.pass === true && alignmentPass && realismPass && score >= 90;
  const issues = Array.isArray(raw.issues) ? raw.issues.map(item => String(item).slice(0, 160)).filter(Boolean).slice(0, 8) : [];
  return json(200, { pass, alignment_pass: alignmentPass, realism_pass: realismPass, score, issues, summary: String(raw.summary || (pass ? '商品一致性、真实感与对位全部通过' : '发现失真或错位问题')).slice(0, 80), correction: pass ? '' : String(raw.correction || raw.summary || '恢复原图主体结构，修复边缘重影、悬浮、穿插、透视错位并重建真实接触阴影').slice(0, 300) });
}
...
export async function handleApiRequest(request, endpoint) {
  ...
  try {
    ...
    if (endpoint === 'download-image') return await downloadImage(input, id);
