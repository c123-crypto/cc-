...
async function analyze(request, input, id) {
  ...
  const includeWhite = suiteType !== 'vertical' && input.includeWhite !== false;
  ...
  const roles = projectSeriesRoles(suiteType, includeWhite);
  ...
  const outputMode = includeWhite
    ? '白底首图由 white_prompt 单独生成，不要把白底图重复放进 series。'
    : '本套不生成白底图，全部输出名额都必须用于彼此不同的真实场景图；white_prompt 必须返回空字符串。';
  const prompt = `你是中国大陆电商百货商品视觉策划师和图像模型提示词专家。必须先完成“多图商品理解”，再为${targetModel}制定逐张可执行的套图方案。\n\n图片分组与顺序：前 ${productImages.length} 张是同一商品的商品证据图，分别可能是正面、侧面、背面、细节、配件或包装，用于综合锁定同一个商品的真实形状、比例、颜色、材质、标签、标识、配件和角度；后 ${styleImages.length} 张是场景/风格参考图，只允许借鉴环境、构图、机位、光线和色调，不能把其中的商品、品牌、文字或人物身份带入结果。\n\n目标平台策略：${PLATFORM_RULES[platform]}\n套图方向：${DIRECTION_RULES[direction]}\n输出策略：${outputMode}\n用户补充：${String(input.userNote || '无').slice(0, 2000)}\n\nseries 必须正好${roles.length}项，顺序和 label 固定为：\n${roleLines}\n\n只输出合法JSON，不要Markdown：\n{"product_name":"不超过14字的准确商品通用名称，不使用文件名和编号","product_summary":"综合全部商品证据图得到的商品身份、结构、材质和可见差异摘要","reference_strategy":"如何把不同商品角度和场景参考分配给整套图","white_prompt":"白底模式填写保真提示词，全场景模式为空字符串","series":[{"label":"固定任务名称","product_refs":[1],"style_ref":1,"prompt":"给${targetModel}的完整中文提示词"}]}\n\nproduct_refs 只能使用 1—${productImages.length}，每项最多2张；style_ref 在有场景图时使用 1—${Math.max(styleImages.length, 1)}，没有则为 null。整套图必须轮换主要商品角度、真实使用环境、道具关系、构图、机位、景别与光线，相邻图片至少四项明显不同，每条提示词写明本张使用的参考分工。每次只生成一张独立单场景图片，禁止拼图、分屏、多宫格、把多个场景塞进一张或仅展示包装。必须保持商品真实，不重新设计、不增减功能、不复制商品、不遮挡主体。画面内禁止新增文字、价格、促销角标、Logo、店铺名、二维码、联系方式、水印、边框、乱码、认证、比较元素和绝对化宣传，不得暗示未经证实的功效或背书。`;
