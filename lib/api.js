...
async function analyze(request, input, id) {
  ...
  const targetModel = input.imageProvider === 'openai' ? 'GPT Image 2' : input.imageProvider === 'gemini' ? 'Nano Banana 2' : input.imageProvider === 'qwen' ? 'Qwen Image 2.0' : 'Seedream 5.0';
...
function imageProviderName(provider) {
  return provider === 'gemini' ? 'Nano Banana 2' : provider === 'openai' ? 'GPT Image 2' : provider === 'qwen' ? 'Qwen Image 2.0' : 'Seedream 5.0';
}
...
async function toolPrompt(request, input, id) {
  ...
  const targetModel = input.provider === 'openai' ? 'GPT Image 2' : input.provider === 'gemini' ? 'Nano Banana 2' : input.provider === 'qwen' ? 'Qwen Image 2.0' : 'Seedream 5.0';
...
async function whiteImage(request, input, id) {
  ...
  try {
    const result = await imageWithFallback(provider, { ark: arkKey, openai: openaiKey, gemini: geminiKey, qwen: qwenKey }, [input.imageBase64], input.prompt, 'square', input, true);
...
async function seriesImage(request, input, id) {
  ...
  try {
    return json(200, await imageWithFallback(provider, { ark: arkKey, openai: openaiKey, gemini: geminiKey, qwen: qwenKey }, [input.whiteImageBase64], input.prompt, aspect, input, true));
...
async function qc(request, input, id) {
  ...
  let raw;
  try {
    const generated = await providerText(textProvider, textKeys, prompt, [input.whiteImageBase64, input.generatedImageBase64], input, { json: true, temperature: 0.1, maxTokens: 800 });
    raw = extractObject(generated.text);
  } catch (error) {
