...
async function remoteJSON(response, service) {
  ...
  try {
    ...
    if (response.ok && raw && contentType.includes('text/plain') && !/^\s*</.test(raw)) {
      return { choices: [{ message: { content: raw } }] };
    }
    const error = new Error(`${service}_FORMAT`);
    error.status = response.ok ? 502 : (response.status || 502);
    error.userMessage = `${service === 'ARK_CHAT' ? '火山方舟' : service}返回了无法识别的数据格式，请检查模型名称、Endpoint和代理配置`;
    throw error;
  }
}
...
function arkResponseText(payload) {
  ...
  for (const item of Array.isArray(payload?.output) ? payload.output : []) {
    for (const content of Array.isArray(item?.content) ? item.content : []) {
      const value = content?.text?.value ?? content?.text ?? content?.output_text;
...
async function arkText(arkKey, prompt, images, model, options = {}) {
  const selectedModel = model || DEFAULT_VISION_MODEL;
  if (String(selectedModel).toLowerCase().includes('deepseek') && !images.length) {
    const response = await timedFetch(ARK_RESPONSES_URL, {
      ...
      body: JSON.stringify({ model: selectedModel, input: prompt, max_output_tokens: options.maxTokens || 2600 }),
    ...
    if (!text) throw Object.assign(new Error('ARK_RESPONSES_EMPTY'), { userMessage: 'DeepSeek 已响应，但没有返回可用文字，请检查该模型是否已开通 Responses API' });
    return text;
  }
...
async function toolText(request, input, id) {
  ...
  const model = images.length
    ? input.visionModel || DEFAULT_VISION_MODEL
    : input.arkTextModel || input.visionModel || DEFAULT_VISION_MODEL;
...
async function connectionTest(request, input, id) {
  ...
  try {
    ...
    if (provider === 'doubao') {
      ...
      const model = input.arkTextModel || input.visionModel || DEFAULT_VISION_MODEL;
