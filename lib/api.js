...
async function geminiText(geminiKey, prompt, images, model) {
  const input = [{ type: 'text', text: prompt }];
  for (const image of images.slice(0, 10)) input.push({ type: 'image', ...dataImage(image) });
  const payload = await geminiRequest(geminiKey, { model: model || DEFAULT_GEMINI_TEXT_MODEL, input, generation_config: { thinking_level: 'low' } }, 90000);
...
async function analyze(request, input, id) {
  ...
  if (textProvider === 'gemini') {
    try { raw = extractObject(await geminiText(geminiKey, prompt, [input.imageBase64], input.geminiTextModel)); }
    catch (error) { return failure(error.status || 502, error.userMessage || 'Gemini提示词规划暂时未完成', error.message || 'GEMINI_ANALYZE_FAILED', id); }
  } else {
  ...
  return json(200, { product_summary: String(raw.product_summary || '已识别商品主体与外观特征').trim().slice(0, 240), white_prompt: String(raw.white_prompt || '只替换为纯白背景，严格保持商品外观、数量、标签和配件。').trim().slice(0, 5000), series, roles, provider: textProvider, model: textProvider === 'gemini' ? input.geminiTextModel || DEFAULT_GEMINI_TEXT_MODEL : input.visionModel || DEFAULT_VISION_MODEL });
}
...
async function toolImage(request, input, id) {
  ...
  try {
    if (provider === 'gemini') {
      if (!geminiKey) return failure(400, '请先填写 Gemini API Key（可使用账户免费额度）', 'GEMINI_KEY_REQUIRED', id);
      return json(200, { image: await geminiImage(geminiKey, images, prompt, input.aspect, input.geminiImageModel), model: input.geminiImageModel || DEFAULT_GEMINI_IMAGE_MODEL, provider: 'gemini' });
    }
    if (provider === 'openai') {
      if (!openaiKey) return failure(400, '请先填写 OpenAI API Key', 'OPENAI_KEY_REQUIRED', id);
      return json(200, { image: await openaiCreate(openaiKey, images, prompt, input.aspect, input.openaiImageModel), model: input.openaiImageModel || DEFAULT_OPENAI_IMAGE_MODEL, provider: 'openai' });
    }
    if (!arkKey) return failure(400, '请先填写火山方舟 API Key', 'ARK_KEY_REQUIRED', id);
    return json(200, { image: await seedreamCreate(arkKey, images, prompt, input.imageModel, input.aspect), model: input.imageModel || DEFAULT_IMAGE_MODEL, provider: 'seedream' });
  } catch (error) {
...
async function toolPrompt(request, input, id) {
  ...
  try {
    if (input.textProvider === 'gemini') {
      ...
      text = await geminiText(geminiKey, prompt, images, model);
    } else {
      ...
      const response = await timedFetch(ARK_CHAT_URL, {
        method: 'POST', headers: { 'Content-Type': 'application/json', Authorization: `Bearer ${arkKey}` },
        body: JSON.stringify({ model, messages: [{ role: 'user', content }], temperature: 0.3, max_tokens: 5200, response_format: { type: 'json_object' } }),
      }, 90000);
      const payload = await remoteJSON(response, 'ARK_CHAT');
      if (!response.ok) { const mapped = classify(remoteMessage(payload), response.status); return failure(mapped.status, mapped.message, mapped.code, id); }
    ...
    const prompts = Array.isArray(raw.prompts) ? raw.prompts.slice(0, outputCount).map((item, index) => ({ label: String(item?.label || `结果 ${index + 1}`).slice(0, 50), prompt: String(item?.prompt || '').trim().slice(0, 12000) })) : [];
    if (prompts.length !== outputCount || prompts.some(item => !item.prompt)) return failure(502, 'AI返回的提示词数量不完整，请重试', 'PROMPT_PLAN_INCOMPLETE', id);
    return json(200, { summary: String(raw.summary || '已完成素材理解与提示词规划').slice(0, 500), prompts, model });
  } catch (error) {
    return failure(error.status || 502, error.userMessage || '提示词生成暂时未完成，请重试', error.message || 'TOOL_PROMPT_FAILED', id);
  }
}
...
async function toolText(request, input, id) {
  ...
  return json(200, { text, model: input.visionModel || DEFAULT_VISION_MODEL, provider: 'doubao' });
}
...
async function whiteImage(request, input, id) {
  ...
  try {
    if (provider === 'gemini') {
      if (!geminiKey) return failure(400, '请先填写 Gemini API Key（可使用账户免费额度）', 'GEMINI_KEY_REQUIRED', id);
      return json(200, { image: await geminiImage(geminiKey, [input.imageBase64], input.prompt, 'square', input.geminiImageModel), model: input.geminiImageModel || DEFAULT_GEMINI_IMAGE_MODEL, provider, isBaseImage: true });
    }
    if (provider === 'openai') {
      if (!openaiKey) return failure(400, '请先填写 OpenAI API Key', 'OPENAI_KEY_REQUIRED', id);
      return json(200, { image: await openaiCreate(openaiKey, [input.imageBase64], input.prompt, 'square', input.openaiImageModel), model: input.openaiImageModel || DEFAULT_OPENAI_IMAGE_MODEL, provider, isBaseImage: true });
    }
    if (!arkKey) return failure(400, '请先填写火山方舟 API Key', 'ARK_KEY_REQUIRED', id);
    return json(200, { image: await seedream(arkKey, input.imageBase64, input.prompt, input.imageModel, 'square'), model: input.imageModel || DEFAULT_IMAGE_MODEL, provider, isBaseImage: true });
...
async function seriesImage(request, input, id) {
  ...
  try {
    if (provider === 'gemini') {
      if (!geminiKey) return failure(400, '请先填写 Gemini API Key（可使用账户免费额度）', 'GEMINI_KEY_REQUIRED', id);
      return json(200, { image: await geminiImage(geminiKey, [input.whiteImageBase64], input.prompt, aspect, input.geminiImageModel), model: input.geminiImageModel || DEFAULT_GEMINI_IMAGE_MODEL, provider, fallback: false });
    }
    if (provider === 'openai') {
      if (!openaiKey) return failure(400, '请先填写 OpenAI API Key', 'OPENAI_KEY_REQUIRED', id);
      return json(200, { image: await openai(openaiKey, input.whiteImageBase64, input.prompt, aspect, input.openaiImageModel), model: input.openaiImageModel || DEFAULT_OPENAI_IMAGE_MODEL, provider, fallback: false });
    }
    if (!arkKey) return failure(400, '请先填写火山方舟 API Key', 'ARK_KEY_REQUIRED', id);
    return json(200, { image: await seedream(arkKey, input.whiteImageBase64, input.prompt, input.imageModel, aspect), model: input.imageModel || DEFAULT_IMAGE_MODEL, provider, fallback: false });
...
async function qc(request, input, id) {
  ...
  let raw;
  if (textProvider === 'gemini') {
    try { raw = extractObject(await geminiText(geminiKey, prompt, [input.whiteImageBase64, input.generatedImageBase64], input.geminiTextModel)); }
